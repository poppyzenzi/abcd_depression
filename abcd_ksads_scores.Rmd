...
This is a script adapted from Shen (insert git link here) to calculate KSADS depression scores from ABCD data
...
library(dplyr)
library(knitr)
#directory where tabulated rds files are stored
tab.dat.dir='/Users/poppygrimes/Library/CloudStorage/OneDrive-UniversityofEdinburgh/Edinburgh/gmm/data'
```
...
```{r}
#note: paste0 used to concatenate vectors after converting to character vectors.
dir.var.ls='ls.questionnaires.csv'
...
#id, date, age, sex, event, ksads score columns
```
...
```{r}
#looping over rows in new dictionary 
for (i in 1:nrow(new.dic.current)){
    #field name, old and new
    tmp.field.current=new.dic.current$Field.name[i]
    tmp.field.past=new.dic.past$Field.name[i]
    #sum across rows from those dictionaries in the 
    tmp.dat.col=rowSums(sapply(targetdat.clean[,c(tmp.field.current,tmp.field.past)],
                                function(x) as.numeric(as.character(x))), na.rm=T)
...
write.csv(targetdat.clean, "/Users/poppygrimes/Library/CloudStorage/OneDrive-UniversityofEdinburgh/Edinburgh/abcd_poppy/data/abcd_ksads.csv", row.names=FALSE)
...
In targetdat.clean (all columns below are life-time measures):
 ...
 - KSADS.MDD.y: MDD diagnosis based on child report  #dont need this
 - KSADS.MDD.p: MDD diagnosis based on caregiver report  #dont need this
 -------------
...
need to decide which scores to use